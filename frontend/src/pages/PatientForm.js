...
<<<<<<< HEAD
    ...
    rec.onstart = () => {
      setIsListening(true);
      setVoiceMode(true);
      toast.success("Assistant Global Actif - Dites par ex: 'Nom Boucham, Prénom Fatima'", { icon: '🤖' });
    };
...
>>>>>>> 601c47f32253b3bce4dd4d8134bce39dbc6bdc58
    rec.onresult = (e) => {
      ...
      // Mode transcription seulement : on affiche le texte dicté sans remplir les champs
    };
...
<<<<<<< HEAD
  ...
  const VoiceInput = ({ label, field, value, type = "text", required = false, list = null, placeholder = "" }) => (
    <div className="form-group">
      <label className="form-label" style={{ fontSize: 12, fontWeight: 900, color: '#94a3b8', textTransform: 'uppercase', marginBottom: 8, display: 'block' }}>
        {label} {required && '*'}
      </label>
      <input
        className="form-control"
        style={{ height: 50, borderRadius: 14, fontWeight: 700, fontSize: 15, border: '1.5px solid #e2e8f0', background: 'white' }}
        type={type}
        value={value}
